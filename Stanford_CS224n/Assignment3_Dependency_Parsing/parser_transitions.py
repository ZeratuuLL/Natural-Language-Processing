...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses : 
        decisions = model.predict(unfinished_parses[:batch_size])
        for i, transition in enumerate(decisions):
            unfinished_parses[i].parse_step(transition)
        unfinished_parses = [parser for parser in unfinished_parses if not (len(parser.buffer)==0 and len(parser.stack)==1)]
